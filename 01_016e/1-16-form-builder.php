...
//helper functions that exist to prevent "undefined" errors. 
...
function build_form( $form ) {
	...
	foreach( $form['fields'] as $id => $field ) {
		...
		switch( $field['type'] ) {
			...
			case 'radio':
				foreach( $field['options'] as $option ) {
					$markup .= '<input type="'. $field['type'].'" id="'. $id .'" name="'. $id .'" value="'. $option .'"><label for="'. $option .'">'. $option .'</label>';
				}
				break;
			case 'checkbox':
				if ( isset( $field['options'] ) ) {
					...
					$markup .= '<input type="'. $field['type'].'" id="'. $id .'" name="'. $id .'" value="'. $field['value'] .'">';
				}
				break;
			default:
